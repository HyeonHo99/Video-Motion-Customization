...
class TextToVideoIFPipeline(DiffusionPipeline, LoraLoaderMixin):
    ...
    def __call__(
        self,
        inv_images: Optional[torch.FloatTensor] = None,
        ...
        # - - - if inverted images exist - - - #
        if inv_images is not None:
            if inv_images.shape != intermediate_images.shape:
                raise ValueError(f"Unexpected inverted latents shape, got {inv_images.shape}, expected {intermediate_images.shape}")
            #inv_images = inv_images * self.scheduler.init_noise_sigma
            intermediate_images = inv_images.to(device)
